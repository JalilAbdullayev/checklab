		...
		<div class="container">
			<nav aria-label="breadcrumb" style="--bs-breadcrumb-divider: url(
				...
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active" aria-current="page">@if(Route::is('search'))
							Products
						@else
							Blogs
						@endif</li>
			...
			<div class="row">
				@foreach($blogs as $blog)
					<div class="col-lg-4 col-md-6 mb-3">
						<a href="@if(Route::is('search')) {{ route('product', $blog->slug) }} @else {{ route('blog.detail', $blog->slug) }} @endif"
						   class="blog-item">
							...
							<div class="blog-item-date">
								@if(Route::is('search'))
									@if($blog->discount)
										{{ $blog->price - (($blog->price * $blog->discount) / 100) }} ₼
									@else
										{{ $blog->price }} ₼
									@endif
								@else
