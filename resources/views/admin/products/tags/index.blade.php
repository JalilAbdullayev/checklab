			...
			<div class="table-responsive">
				<table id="myTable" class="table table-striped border">
					...
					@foreach($data as $item)
						<tr id="{{ $item->id }}">
							<td>
								<a href="{{ route('admin.products.tag.all', $item->slug) }}" class="text-body">
									{{ $item->title }}
								</a>
							</td>
							<td>
								<a href="{{ route('admin.products.tag.edit', $item->id) }}" class="btn
                                btn-outline-warning">
...
@section('js')
	...
	<script>
        ...
        $('.btn-outline-danger').click(function() {
            ...
            $.ajax({
                url: '{{ route('admin.products.tag.delete', ':id') }}'.replace(':id', id),
                async: false,
                success: function() {
                    $('tr#' + id + '').remove();
                },
