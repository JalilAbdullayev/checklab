        ...
        <div class="card-body">
            <div class="form-floating mb-3">
                <input type="text" class="form-control" name="title" id="title" placeholder="Başlıq" required
                       maxlength="255" value="{{ $product->title }}"/>
            ...
            <div class="mb-3">
                ...
                image/png, image/jpg, image/gif, image/svg" data-show-remove="false" @if($product->image)
                    data-default-file="{{ asset(Storage::url($product->image)) }}" @endif/>
            </div>
            @error('image')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
